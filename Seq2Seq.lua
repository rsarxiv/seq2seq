...
local Seq2Seq = torch.class("Summarization.Seq2Seq")
...
function Seq2Seq:train(encoderInput,decoderInput,decoderTarget)
  local decoderTarget = nn.SplitTable(1,1):forward(decoderTarget)
  -- Forward pass
...
function Seq2Seq:eval(input)
  assert(self.bosToken, "No goToken specified")
  ...
  -- Forward <s> and all of it's output recursively back to the decoder
  local output = self.bosToken
